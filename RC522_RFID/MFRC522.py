...
'''
Edited by Sangmork "SAM" Park on July-2024
 @ Original MFRC Library depend on RPi.GPIO library which does not support the gpiozero implementation
 
 ++++ Changed as below ++++
 1. change rst_pin implementation
    - commented out all GPIO.xxx codes and related codes which setup the rst_pin
    - added codes below: only BCM pin mode is supported
    
    +++
    if pin_rst == -1:
        pin_rst = gpiozero.DigitalOutputDevice(pin=15)
        pin_rst.on()
    +++  
'''
...
class MFRC522:
    ...
    def __init__(self, bus=0, device=0, spd=1000000, pin_mode=10, pin_rst=-1, debugLevel='WARNING'):
        ...
        #---------- Edited by SAM ---------------------------------------------- 
        # GPIO pin_mode == 10 --> GPIO.BOARD, pin_mode == 11 --> GPIO.BCM
        # pin_mode = BCM --> set pin_rst = 15 by using DigitalOutDevice() class 
        # which is a grgular class for digital output
        if pin_rst == -1:
            pin_rst = gpiozero.DigitalOutputDevice(pin=15)
            pin_rst.on()
        #-----------------------------------------------------------------------
